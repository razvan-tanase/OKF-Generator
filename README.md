...
- **Stage 09 — Compile:** complete. One verified Stage 08 plan is deterministically applied to an immutable canonical-state generation, with stable identity finalization, event history, generated Stage 07/08 projections, stale-plan protection, and an atomically replaced current pointer.
- **Stage 10 — Structuralize:** complete. Verified canonical state is projected into a deterministic LLM-Wiki / OKF v0.1 structural IR with typed documents, stable public paths, private identity mapping, and explicit Stage 13/14 derivation/serialization boundaries.
- **Stage 11+**: planned and intentionally not implemented yet.

See [`workflow/stages.yaml`](workflow/stages.yaml) for the complete stage graph, [`docs/stage-01-acquire.md`](docs/stage-01-acquire.md) for acquisition, [`docs/stage-02-snapshot.md`](docs/stage-02-snapshot.md) for snapshotting, [`docs/stage-03-classify.md`](docs/stage-03-classify.md) for classification, [`docs/stage-04-extract.md`](docs/stage-04-extract.md) for extraction, [`docs/stage-05-normalize.md`](docs/stage-05-normalize.md) for normalization, [`docs/stage-06-synthesize.md`](docs/stage-06-synthesize.md) for synthesis, [`docs/stage-07-resolve.md`](docs/stage-07-resolve.md) for identity resolution, [`docs/stage-08-plan.md`](docs/stage-08-plan.md) for change planning, [`docs/stage-09-compile.md`](docs/stage-09-compile.md) for canonical-state compilation, and [`docs/stage-10-structuralize.md`](docs/stage-10-structuralize.md) for OKF structuralization.

## Development

Requires Python 3.11+; Git is additionally required for Git acquisition, Git snapshot locking, and Git source extraction. Stage 04 pins its Python extraction dependencies in `pyproject.toml`; Stages 05–10 add no runtime Python dependency. The Stage 06 OpenAI adapter calls the Responses API over the standard library HTTPS client and reads `OPENAI_API_KEY` from the environment; Stage 07 optionally reuses that adapter only for unresolved identity adjudication. Stages 08–10 make no model call in `builtin-v1`. Stage 10 keeps OKFy and OpenWiki as pinned compatibility probes rather than runtime dependencies.
...
okf-generator compile paper sha256-<snapshot-digest> sha256-<synthesis-run> sha256-<resolution-run> sha256-<plan-run> --synthesis-provider openai
